class PotentialSubjectsController < ApplicationController
  layout "standard"
  active_scaffold :subject do |config|
    config.theme = :blue
    config.label = "Generation R scheduling"
  ...
  def conditions_for_collection
    # set excluded study ids
    #excluded_studies = [101,102,103]
    ...
    nij_subjects = []
    Study.find_by_id(102).enrollments.each {|y| nij_subjects << y.subject_id}
    att_subjects = []
    Study.find_by_id(103).enrollments.each {|y| att_subjects << y.subject_id}
    lei_subjects = []
    Study.find_by_id(104).enrollments.each {|y| lei_subjects << y.subject_id}
    ...
    @potential_subjects = subjects - scheduled_subjects - completed_subjects - unwilling_subjects - neo_subjects - nij_subjects - att_subjects - lei_subjects - no_calls
