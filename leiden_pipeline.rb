...
#######################################################################################
# This script is used to run the preprocessing pipeline for the Leiden study
#
...
if ARGV.length < 1
  ...
  puts " Example: ./leiden_pipeline.rb 100123"
  puts " "
  puts " Note: You can include multiple event_ids, just separate each one with a space."
  puts " Example: ./leiden_pipeline.rb 100123 100136 101458"
  puts " "
  exit
end
...
ARGV.each do |event_id|
  ...
  # preprocess hondentaak run 1 fMRI
  fmri_basic_preproc(event_id,13)

  # preprocess hondentaak run 2 fMRI
  fmri_basic_preproc(event_id,14)
  ...
  system("/Volumes/vgraid/mr_data/scripts/error_chk_leiden.sh #{r_number} #{event_id} #{study}")
end

if ARGV.length > 1
  ARGV.each do |event_id|
    system("/Volumes/vgraid/mr_data/scripts/batch_error_chk_leiden.rb #{event_id}")
  end
end
