class LeidenSchedulingsController < ApplicationController
  layout "standard"
  active_scaffold :subject do |config|
    config.theme = :blue
    config.label = "Leiden scheduling"
  ...
  def conditions_for_collection
    # filter to specific study_id
    study = 104
    # get all the subjects in mayo
    subjects = []
...
#Event.find(:all, :conditions => ["event_status < 2 OR event_status > 3"]).each {|g| good_events << g.id}
    Event.find(:all, :conditions => ["event_status != 2 AND event_status != 3 AND event_status != 10 AND event_status != 11"]).each {|g| good_events << g.id}
    #Event.find(:all, :conditions => ["event_status != 2"]).each {|g| good_events << g.id}
